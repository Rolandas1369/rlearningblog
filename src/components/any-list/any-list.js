...
const Record = (props) => {
  ...
  return (
    <span>
      {props.children.id}. {props.children.content} {props.children.date_added}
    </span>
  );
};
...
export default class AnyList extends Component {
  ...
  build_list(user) {
    ...
    let order = featArray.map((element) => {
      return (
        <span className="list-item" key={element.id}>
          ...
          {user ? (
            <button
              onClick={() =>
                this.handleDelete(element.id, featArray, this.props.feature)
              }
            >
              Delete{" "}
            </button>
